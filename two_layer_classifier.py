...
class TwoLayerClassifier(object):
    ...
    def predict(self, x):
        ...
        if len(x.shape) == 1:  # Predict on one sample
            ...
            # CORRECTION: Le calcul du softmax est inutile et couteux en temps de calcul
            # -0.25
            expo = np.exp(scores)
            S = expo / np.sum(expo)
            ...
            for i in range(x.shape[0]):  # on ittere sur les samples de x
                scores = self.net.forward(x[i, :])
                expo = np.exp(scores)
                S = expo / expo.sum()
                predictions[i] = np.argmax(S)
            return predictions
            #############################################################################
            #                          END OF YOUR CODE                                 #
            #############################################################################
    ...
    def global_accuracy_and_cross_entropy_loss(self, x, y, l2_r=-1.0):
        ...
        for x_i, y_i in zip(x, y):
            scores = self.net.forward(x_i)  # forward pass
            loss_i, dloss_dscores_i = self.net.cross_entropy_loss(scores, y_i)
            loss = loss + loss_i
...
class TwoLayerNet(object):
    ...
    def cross_entropy_loss(self, scores, y):
        ...
        Lreg = self.l2_reg * sum(np.sum(parameter**2)
...
class DenseLayer(object):
    ...
    def reinit(self):
        self.W = np.random.randn(
            self.in_size + 1, self.out_size) * np.sqrt(0.1 / (self.in_size + self.out_size))
