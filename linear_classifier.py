...
class LinearClassifier(object):
    ...
    def train(self, num_epochs=1, lr=1e-3, l2_reg=1e-4, lr_decay=1.0, init_scale=0.01):
        ...
        for i in range(num_iter):
            ...
            if sample_idx >= len(self.x_train):  # End of epoch
                ...
                accu_train, loss_train = self.global_accuracy_and_cross_entropy_loss(
                    self.x_train, self.y_train, l2_reg)
                accu_val, loss_val, = self.global_accuracy_and_cross_entropy_loss(
    ...
    def predict(self, X):
        ...
        a = np.dot(X, self.W)
        class_label = np.argmax(a, axis=1)
    ...
    def global_accuracy_and_cross_entropy_loss(self, X, y, reg=0.0):
        ...
        accu = (prediction == y).mean()

        # average cross-entropy
        for i in range(len(X)):
            loss_i, dw_i = self.cross_entropy_loss(X[i], y[i], reg)
            loss = loss + loss_i
    ...
    def cross_entropy_loss(self, x, y, reg=0.0):
        ...
        # calcul de Softmax
        # produit scalaire entre le vecteur en entree x et la matrice de poids W
        a = np.dot(x, self.W)
        a = a - np.max(a)  # soustraction du maximum pour eviter l'instabilite
        expo = np.exp(a)  # expo des scores
        S = expo / expo.sum()  # on passse les scores par la fonctions Softmax

        # creation du onehotVector
        t = np.zeros((y.size, self.num_classes))
        t[np.arange(y.size), y] = 1

        # calcul du gradient
        dW = np.dot(x[np.newaxis].T, (S - t)) + reg * self.W

        # Calcul de cross_entropy loss
        Lpred = -np.log(S[y])
        Lreg = reg * np.sum(self.W ** 2)  # ajout de la regularisation
        loss = Lpred + Lreg
